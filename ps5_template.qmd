...
two_split = df_2021[df_2021['Category'].isin(["Criminal and Civil Actions", "State Enforcement Agencies"])]

# NOT SURE
two_split = two_split.groupby('Category').size().reset_index(name='Count')

alt.Chart(two_split).mark_line().encode(
    x='Category:O',
    y='Count:Q'
).properties(
...
five_topics = df_2021[df_2021['Category'].isin(["Criminal and Civil Actions"])]

topics = []

for title in five_topics['Title']:
  title = title.lower()  # Convert to lowercase for easy matching

  # checking key words
  if "health care" in title or "health" in title:
...
# append topic column
five_topics['topics'] = topics

# NOT SURE
five_topics = five_topics.groupby('topics').size().reset_index(name='Count')

alt.Chart(five_topics).mark_line().encode(
    x='topics:O',
    y='Count:Q'
).properties(
    title= 'Five Topics',
